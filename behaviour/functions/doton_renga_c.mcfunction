...
execute as @e[tag=terra,scores={chakra=40..}] ~~~ fill ^-6 ^-3 ^3 ^6 ^7 ^3 dirt_with_roots

execute as @s[scores={tp=400..}] ~~~ replaceitem entity @s slot.weapon.mainhand 1 porkyn:doton_renga_b 1 0 {"item_lock":{"mode":"lock_in_inventory"},"keep_on_death":{}}
execute as @s[scores={tp=400..}] ~~~ playsound lvl @s
execute as @s[scores={tp=400..}] ~~~ scoreboard players remove @s tp 400
